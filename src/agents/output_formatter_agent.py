...
class OutputFormatterAgent:
    ...
    def export_to_excel(
        ...
        # Sheet 1: DQ Rules (main table)
        ws_rules = wb.active
        ws_rules.title = "DQ Rules"
        self._write_rules_sheet(ws_rules, rules)

        # Sheet 2: Summary by Category
        ws_summary = wb.create_sheet("Summary by Category")
        self._write_summary_sheet(ws_summary, rules)
        ...
        self._write_sql_sheet(ws_sql, rules)

        # Sheet 5: Python Implementations
        ws_python = wb.create_sheet("Python Implementations")
        self._write_python_sheet(ws_python, rules)

        # Sheet 6: Rules by Attribute
        ws_by_attr = wb.create_sheet("Rules by Attribute")
        self._write_by_attribute_sheet(ws_by_attr, rules)

        wb.save(output_path)
        return str(output_path)

    def _write_rules_sheet(self, ws, rules: List[DQRule]) -> None:
        """Write main rules table to worksheet."""
        headers = [
            "Rule ID", "Attribute", "Category", "Rule Type", "Expression",
        ...
        for row, rule in enumerate(rules, 2):
            ...
            # Severity with color
            severity_cell = ws.cell(row=row, column=6, value=rule.severity)
            ...
            ws.cell(row=row, column=7, value=rule.description)
            ws.cell(row=row, column=8, value=rule.threshold_percent)
            ws.cell(row=row, column=9, value=rule.confidence_score)
            ws.cell(row=row, column=10, value=rule.derived_from)
            ws.cell(row=row, column=11, value=", ".join(rule.sample_valid_values[:5]))
            ws.cell(row=row, column=12, value=", ".join(rule.sample_invalid_values[:5]))
        ...
        column_widths = {
            'A': 45, 'B': 30, 'C': 15, 'D': 18, 'E': 60,
            'F': 12, 'G': 50, 'H': 12, 'I': 12, 'J': 40,
            'K': 30, 'L': 30
        }
    ...
    def _write_summary_sheet(self, ws, rules: List[DQRule]) -> None:
    ...
    def _write_sql_sheet(self, ws, rules: List[DQRule]) -> None:
        """Write SQL implementations."""
        headers = ["Rule ID", "Attribute", "Category", "SQL Expression"]
    ...
    def _write_python_sheet(self, ws, rules: List[DQRule]) -> None:
        """Write Python implementations."""
        headers = ["Rule ID", "Attribute", "Category", "Python Expression"]
    ...
    def _write_by_attribute_sheet(self, ws, rules: List[DQRule]) -> None:
        """Write rules grouped by attribute."""
        ...
        headers = ["Attribute", "Rule Count", "Categories", "Severities", "Rule IDs"]
        ...
        for row, (attr, attr_rules) in enumerate(sorted(by_attr.items()), 2):
            ...
            ws.cell(row=row, column=2, value=len(attr_rules))
            ws.cell(row=row, column=3, value=", ".join(categories))
            ws.cell(row=row, column=4, value=", ".join(severities))
            ws.cell(row=row, column=5, value=", ".join(rule_ids))
