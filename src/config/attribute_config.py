...
# Set to 0 to disable filtering (process all attributes regardless of missing %)
MAX_MISSING_THRESHOLD: float = 0.0
...
def should_skip_for_missing(missing_percentage: float) -> bool:
    ...
    Returns:
        True if missing > MAX_MISSING_THRESHOLD. Returns False if threshold is 0 (disabled).
    """
    if MAX_MISSING_THRESHOLD == 0:
        return False
